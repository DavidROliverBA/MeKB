...
def parse_skill(path):
    ...
    command = None
    match = re.match(r"# /(\w+)", lines[0]) if lines else None
    if match:
...
def list_skills(vault_root):
    ...
    print(f"  {'Command':<18} {'File':<22} {'Lines':>5}  {'Sections':>8}  Scripts")
    ...
    for path in skills:
        ...
        print(f"  {cmd:<18} {info['filename']:<22} {info['lines']:>5}  {info['sections']:>8}  {scripts}")
...
def validate_skill(path, vault_root, verbose=False):
    ...
    if info["command"] and info["command"] != path.stem:
        issues.append(f"Command '/{info['command']}' doesn't match filename '{path.stem}.md'")
...
def export_skill(skill_name, vault_root):
    ...
    if not skill_path.exists():
        ...
        print(f"Available: {', '.join(p.stem for p in skills_dir.glob('*.md'))}")
        sys.exit(1)
    ...
    # Copy skill file
    shutil.copy2(skill_path, export_dir / f"{skill_name}.md")
...
def main():
    ...
    if args.action == "list":
        ...
        if args.target:
            ...
            skill_path = get_skills_dir(vault_root) / f"{skill_name}.md"
            if not skill_path.exists():
                print(f"Skill not found: {skill_name}")
                sys.exit(1)

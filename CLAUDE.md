...
**Session context:** See `.claude/AGENDA.md` for current focus, pending decisions, and session notes.
...
Use the `> [!ai-suggestion]` callout to mark AI-generated content blocks that need human review:

```markdown
> [!ai-suggestion] Claude Code — 2026-02-13
> This section was generated by Claude and has not been reviewed.

The proposed architecture uses an event-driven pattern...
```

Use `/de-ai-ify` to clean up AI-generated text after review. Remove the callout once content has been verified.

## Naming Conventions

### Claim-Based Naming (Optional)

For Note and Concept types, prefer **claim-based titles** that embed a thesis rather than a topic:

| Instead of | Use |
|-----------|-----|
| `Note - Thoughts on AI Slop` | `Note - Quality Is the Hard Part` |
| `Concept - Voice Latency` | `Concept - Sub-200ms Voice Latency Requires Edge Processing` |

Descriptive titles remain fine for early-stage ideas where no claim has formed yet.
...
MeKB/
├── .claude/
│   └── skills/        # Claude Code skills (40 skills)
